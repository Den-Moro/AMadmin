...
- [x] Админ-панель: логин с блокировкой после неудачных попыток, роли
  (operator/administrator/superadmin), дашборд ПК, оповещения, группы, мануалы,
  команды, настройки — у каждого поля подсказка
