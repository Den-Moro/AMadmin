...
Дата среза: 25.09.2026, последний коммит `02efd5a`.
...
```
server/
├── Core/            Config, Db, Settings, Auth (агенты), AdminAuth (панель), Logger, Router,
│                    PageRouter (чистые URL панели, см. §3), TargetMatcher
├── Modules/<Фича>/  Auth, Dashboard (ПК+статистика), Notifications, Commands, Groups,
│                    Manuals, Meta (справочники), Settings, Logs
├── Views/admin/     HTML-шаблоны страниц панели (index.html, login.html, <раздел>/<раздел>.html) —
│                    вне public/, чтобы не отдавались напрямую как статика (см. §3, PageRouter)
├── public/
...
├── migrations/      001…014, хронологические, накатываются bin/migrate.php
...
Панель (тёмная тема по умолчанию, светлая по кнопке; живое авто-обновление раз в 15–30 с
на всех списочных страницах, лёгкие CSS-анимации под `prefers-reduced-motion`):
- **Дашборд** — сводка: парк, активность за сутки, магазины, версии агентов (+ тайл
  «устаревшие агенты»), события, «требуют внимания» (с поиском по хосту/магазину/IP);
- **Хосты** — фильтры (магазин/тип/группа/статус/версия), сортировка по клику на
  заголовок (`Ui.makeSortable`, теперь переиспользуется и в «Командах»), IP-адрес
  колонкой (копирование в один клик), страницы, массовые действия, профиль хоста с
  историей и бейджами успех/ошибка/надёжность;
- **Оповещения** (+ подтверждения по кассам, отзыв), **Группы** — обе формы получили
  необязательные поля «свой бренд/контакт» (переопределяют общий из «Настроек» —
  приоритет группа > магазин > общий, см. `OccurrencesController::resolveBrand`),
  **Мануалы**;
- **Команды** — 4 типа: службы, процессы, скрипты, файлы (с хешем и фоновой загрузкой);
  выбор конкретного ПК и в командах, и в оповещениях — теперь через `Ui.pcPicker`
  (поиск по имени/магазину/IP вместо голого `<select>` на сотни строк);
- **Справочники** (магазины получили те же поля бренда, что и группы), **Логи** (живой
  хвост app.log), **Настройки** (3 вкладки; вкладка «Сервер» — только superadmin),
  **Пользователи**.
- **URL панели — «чистые»**: `/admin/hosts` вместо `/admin/hosts/hosts.html` и т.п.
  (см. `server/Core/PageRouter.php`); старые `.../*.html`-ссылки 301-редиректят на новые,
  с сохранением query-строки. Нюанс на будущее: у части страниц чистый путь совпадает с
  path своего же JSON-эндпоинта списка (`GET /admin/stores` — и страница, и API) —
  разводится по заголовку `Accept` (браузерная навигация шлёт `text/html`, `fetch()` из
  `api.js` — нет); любой новый скрипт/curl-проверка, бьющая в такой путь напрямую,
  должна явно слать `Accept: text/html`, иначе получит JSON 401/404, а не страницу
  (уже поймано и исправлено в `install-server.ps1`/`.sh`).

Клиент: оповещения с защитой от случайного закрытия на сенсорных кассах, команды всех
четырёх типов, ограничение скорости и асинхронная загрузка файлов, окно «Настройки» —
теперь растягивается (было фиксированного размера), в трее (путь установки, правка
config.json, импорт конфига файлом, выбор своей `.ico` для значка в трее) — UiAgent
применяет изменения сразу, ManagementAgent (служба) требует перезапуска администратором.
Новый пункт трея «Последние команды» — локальный журнал последних 20 выполненных команд
(`client/Core/RecentCommands.cs`), не зависит от сети/сервера.
...
- Чистые URL (25.09): все 12 страниц панели проверены живым браузером — открываются по
  прямому хиту (не только клиентской навигацией), старые `.html`-ссылки с query-строкой
  301-редиректят корректно, назад/вперёд в браузере не ломаются, `router.php` (Native)
  ведёт себя так же, как nginx. Семь страниц, у которых чистый путь совпадал с путём
  собственного JSON-списка (магазины/оповещения/мануалы/команды/логи/настройки/
  пользователи), отдельно перепроверены после фикса — грузят свои данные, консоль чистая.
